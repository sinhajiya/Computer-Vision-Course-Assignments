...
class OpticalFlow:
    def __init__(self, video_path, output_path="output.avi", window_size=5, bg_frames=50, fps=30):
        ...
        self.cap = cv2.VideoCapture(video_path)    #opening the inout video
    ...
    def initialize_background(self):  #set background of the video as the median of the first 50 (Default) frames
        frames = []
    ...
    def grad_matrix(self, prev_frame, current):
        ...
        It = current - prev_frame # It stores the difference between 2 consecutive frames
    ...
    def lucas_kanade(self, prev_frame, current, window_size):
        ...
        Ix, Iy, It = self.grad_matrix(prev_frame, current)  # get the gradient matrix of the prev frame
        ...
        for i in range(wsizeby2, prev_frame.shape[0] - wsizeby2):
            for j in range(wsizeby2, prev_frame.shape[1] - wsizeby2):
                ...
                B = It[i - wsizeby2:i + wsizeby2 + 1, j - wsizeby2:j + wsizeby2 + 1].flatten()
            ...
            # calculating u matrix
                if np.linalg.det(AtA) != 0:
                    ...
                    h_vel[i, j] = 0  # or use a small value like 1e-3 to prevent invalid entries
                    v_vel[i, j] = 0
